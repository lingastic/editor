----- Overview of positioning approach for modules, lessons, parts, and activities
-- 1. **Add `position` fields**: Add an `position` column to all the necessary tables.
-- 2. **Initialize `position` values**: Initialize the `position` values for existing records using SQL scripts.
-- 3. **Create functions**: Write functions to handle reordering on update and setting initial `position` values on insert.
-- 4. **Create triggers**: Create triggers to call these functions on insert and update operations.

-- With this setup, we ensure that elements in all tables (`modules`, `lessons`, `parts`, and `activities`) maintain their correct position within their respective parent contexts. Each new row gets the maximum `position` value plus one, and reordering happens correctly when updating existing rows.
...
-- Initialize `position` values example

-- For modules
-- WITH ranked_modules AS (
    -- SELECT id, level_id, ROW_NUMBER() OVER (PARTITION BY level_id ORDER BY id) AS position FROM modules
-- )
-- UPDATE modules SET position = ranked_modules.position
-- FROM ranked_modules
-- WHERE modules.id = ranked_modules.id;

-- ### Modules Table

-- Set position on insert
